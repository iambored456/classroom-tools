...
const SLOT_COLOR_SETS = {
  2: ['#ef8aab', '#2dc8b1'],
  3: ['#ef8aab', '#a4ba57', '#58b8f6'],
  4: ['#ef8aab', '#cdaa42', '#2dc8b1', '#8fa9ff'],
  5: ['#ef8aab', '#dfa049', '#7ac379', '#24c1e1', '#aaa0fa'],
  6: ['#ef8aab', '#e89955', '#a4ba57', '#2dc8b1', '#58b8f6', '#ba9bf2'],
  7: ['#ef8aab', '#ed955f', '#bdb147', '#66c589', '#0cc4d5', '#79b0fd', '#c597eb'],
  8: ['#ef8aab', '#f09368', '#cdaa42', '#8ac06c', '#2dc8b1', '#39beea', '#8fa9ff', '#cc95e5'],
  9: ['#ef8aab', '#f2916f', '#d8a444', '#a4ba57', '#5ac692', '#05c5cd', '#58b8f6', '#9ea4fd', '#d193e0'],
  10: ['#ef8aab', '#f38f74', '#dfa049', '#b6b44b', '#7ac379', '#2dc8b1', '#24c1e1', '#6fb2fc', '#aaa0fa', '#d591db'],
  11: ['#ef8aab', '#f38e79', '#e49c4f', '#c3ae44', '#92be66', '#52c798', '#08c6c9', '#42bdee', '#81adfe', '#b39df6', '#d990d7'],
  12: ['#ef8aab', '#f48e7d', '#e89955', '#cdaa42', '#a4ba57', '#6ec482', '#2dc8b1', '#16c3da', '#58b8f6', '#8fa9ff', '#ba9bf2', '#db8fd4'],
  13: ['#ef8aab', '#f48d81', '#eb975a', '#d5a643', '#b2b54d', '#84c171', '#4dc79c', '#0cc7c5', '#31bfe7', '#6ab4fb', '#9aa6fe', '#c099ef', '#dd8fd1'],
  14: ['#ef8aab', '#f48d84', '#ed955f', '#dba345', '#bdb147', '#96bd63', '#66c589', '#2dc8b1', '#0cc4d5', '#47bcf0', '#79b0fd', '#a3a3fc', '#c597eb', '#df8ecf'],
  15: ['#ef8aab', '#f48c86', '#ef9464', '#dfa049', '#c6ad43', '#a4ba57', '#7ac379', '#49c79e', '#11c7c3', '#24c1e1', '#58b8f6', '#85acfe', '#aaa0fa', '#c996e8', '#e08ecc'],
  16: ['#ef8aab', '#f48c89', '#f09368', '#e39d4d', '#cdaa42', '#afb64f', '#8ac06c', '#5fc68e', '#2dc8b1', '#07c5d1', '#39beea', '#67b4fa', '#8fa9ff', '#b09ef7', '#cc95e5', '#e18dca'],
};
...
const stopAlarmSound = () => {
  if (timerState.alarmNode) {
    try {
      timerState.alarmNode.stop();
    } catch {}
    try {
      timerState.alarmNode.disconnect();
    } catch {}
...
const startTimerEditing = () => {
  if (timerState.running || timerEditing || !timerDisplay || !timerCircle) return;
  timerEditing = true;
...
const stopTimerEditing = (commit) => {
  if (!timerEditing || !timerDisplay || !timerCircle) return;
  timerEditing = false;
  stopAlarmSound();
  timerDisplay.setAttribute('contenteditable', 'false');
...
const handleTimerDisplayInput = () => {
  if (!timerEditing || !timerDisplay) return;
...
const beginTimerResize = (event) => {
  if (!(event instanceof PointerEvent) || !timerPanel) return;
  ...
  if (timerResizeHandle?.setPointerCapture) {
    try {
      timerResizeHandle.setPointerCapture(event.pointerId);
    } catch {}
  }
...
const endTimerResize = (event) => {
  ...
  if (timerResizeHandle?.releasePointerCapture && event) {
    try {
      timerResizeHandle.releasePointerCapture(event.pointerId);
    } catch {}
  }
...
const lightenColor = (hex, amount = 0.2) => {
  ...
  if (normalized.length !== 6 || Number.isNaN(parseInt(normalized, 16))) return hex || '#7c91ff';
  ...
  const mix = (value) => {
    ...
    return clampByte(value + value * amount);
  };
...
const getPalette = (count) => {
  if (SLOT_COLOR_SETS[count]) return SLOT_COLOR_SETS[count];
  if (count > 16) {
    const base = SLOT_COLOR_SETS[16];
    return Array.from({ length: count }, (_, idx) => base[idx % base.length]);
  }
  const keys = Object.keys(SLOT_COLOR_SETS)
    .map(Number)
    .sort((a, b) => a - b);
  const fallback = keys.reduce((acc, cur) => (cur <= count ? cur : acc), 16);
  return SLOT_COLOR_SETS[fallback] || SLOT_COLOR_SETS[16];
};
...
const applyZoom = () => {
  const scale = Math.min(1.6, Math.max(0.1, settings.zoomScale || 1));
  settings.zoomScale = scale;
  document.documentElement.style.setProperty('--zoom-scale', scale);
  document.documentElement.style.setProperty('--schedule-zoom', scale);
  if (panelSchedule) {
    panelSchedule.style.setProperty('--schedule-zoom', scale);
  }
  if (zoomResetBtn) {
...
const adjustZoom = (delta) => {
  ...
  const next = Math.min(1.6, Math.max(0.1, Math.round(raw * 100) / 100));
...
const loadUIFromSettings = () => {
  ...
  if (titleTextInp) titleTextInp.value = settings.title;
  if (startTimeInp) startTimeInp.value = settings.startTime;
  if (autoChainChk) autoChainChk.checked = !!settings.autoChain;
  if (iconSizeSel) iconSizeSel.value = settings.iconSize || 'medium';
  if (timeFormatSel) timeFormatSel.value = settings.timeFormat;
  if (showSecChk) showSecChk.checked = !!settings.showSeconds;
  if (offsetInp) offsetInp.value = formatOffset(settings.clockOffsetMs || 0);
  if (alarmStatusEl) {
...
const renderSchedule = () => {
  ...
  const palette = getPalette(scheduleDay.activities.length || 1);
  let nextUp = null;
  ...
  scheduleDay.activities.forEach((act, index) => {
    ...
    const trackColor = lightenColor(baseColor, -0.55);
    const softColor = lightenColor(baseColor, 0.25);
    const brightColor = lightenColor(baseColor, 0.4);

    const card = document.createElement('div');
    card.className = 'card';
    card.setAttribute('role', 'listitem');
    card.style.setProperty('--slot-color', baseColor);
    ...
    titleEl.textContent = act.title || 'Untitled activity';
    ...
    const trackShade = state === 'done' ? softColor : trackColor;
    progressWrap.style.background = trackShade;
    ...
    startTag.style.background = softColor;
    ...
    endTag.style.background = softColor;
    progressWrap.appendChild(endTag);

    const bar = document.createElement('div');
    bar.className = 'bar';
    const progressPercent = Math.max(0, Math.min(100, progress * 100));
    bar.style.setProperty('--w', `${progressPercent.toFixed(3)}%`);
    const midpointColor = lightenColor(baseColor, 0.12);
    bar.style.background = `linear-gradient(90deg, ${midpointColor}, ${brightColor})`;
    ...
    if (state === 'in-progress') {
      ...
      remainingLabel.style.background = baseColor;
      remainingLabel.style.color = '#050a15';
      const rawPosition = progressPercent;
      ...
      if (rawPosition < 12) startTag.classList.add('hidden');
      if (100 - rawPosition < 12) endTag.classList.add('hidden');
    } else if (state === 'done') {
      ariaText = 'Complete';
    } else {
      ariaText = 'Upcoming';
